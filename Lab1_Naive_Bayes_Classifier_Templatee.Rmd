...
### Pavlo Kryven, Baranovska Tetiana, Hrodzytska Anita
...
In conclusion we can say that such classifier can be very powerful tool
for specific datasets. Why specific? Because as was examined throughout
the writing of this lab work, the quality of data was prooved to be very
important. For instance, in given dataset was much more neutral entries,
than discriminatic, which at first glance shouldn't be a problem. But as
was seen in the probabilistic distributions, even with using Laplace
smoothing words for the discriminating class were evaluated differently
to those in neutral class. Also there was a big issues with the
different kinds of stop words and mistakes in words coding. Those could
be addressed applying lemmatization of words. Overal performance of
model for user input is quite good, and can consistently show accurate
classes.

The biggest limitation for us came with the explicit use of R language,
that don't provide such vectorazation possibilities as C or C++. This is
the main reason of such slow work of classification. Talking about Naive
Bayes model, it performed well better than was expected. Because of the
Bayes theorem which account number of entries for each class, it still
managed to get quite good aproximation besides such uneven training
dataset. Also this model is really basic to implement, unlike some other
classification models which use complex methods. Because of our strong
assumption of independence of words within tweets, we achived some pros
and cons for our model. It obviously became more simple and
straightforward to calculate general conditional probability for
feature. However, such assumption lead us to losing some accuracy, as we
know, that words and their order, matter. In general, it makes sense to
watch at words that was used, and based on how frequently they are used
in neutral or discriminatory cases, make assumption about given
sentence.
